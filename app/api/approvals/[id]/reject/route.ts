...
const bodySchema = z.object({
  comment: z.string().min(10, 'Rejection reason must be at least 10 characters').max(2000),
})
...
export async function POST(request: NextRequest, { params }: { params: Params }) {
  ...
  if (!String(req.status).startsWith('pending_l')) {
    return NextResponse.json(
      apiError(`Cannot reject a request with status "${req.status}"`, 'INVALID_STATUS'),
      { status: 400 }
    )
  }
  ...
  if (caller.role !== 'admin' && caller.role !== 'group_admin' && caller.role !== 'procurement_officer') {
    ...
    if (caller.role !== expectedRole) {
      return NextResponse.json(
        apiError('Not authorized to reject at this level', 'FORBIDDEN'),
        { status: 403 }
      )
    }
  }
  ...
  if (!result.success) {
    return NextResponse.json(apiError(result.error ?? 'Rejection failed', 'APPROVAL_ERROR'), { status: 400 })
  }
